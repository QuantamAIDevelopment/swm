...
@app.get("/clusters", summary="Get cluster data in GeoJSON format", description="Get cluster boundaries, routes, and depots as GeoJSON FeatureCollection")
async def get_clusters():
    """Return cluster data as GeoJSON FeatureCollection with routes, cluster outlines, and depot points."""
    try:
        ...
        building_centroids = []
        for pt in buildings_projected.geometry.centroid:
            pt_wgs84 = gpd.GeoSeries([pt], crs='EPSG:3857').to_crs('EPSG:4326').iloc[0]
            building_centroids.append((pt_wgs84.x, pt_wgs84.y))
        
        # Validate we have buildings for clustering
        if len(building_centroids) == 0:
            raise ValueError("No building centroids found for clustering")
        ...
        n_clusters = min(n_clusters, len(building_centroids))  # Can't have more clusters than buildings
        ...
        # Create road network from points only
        G = nx.Graph()
        cluster_road_points = []
        if roads_gdf is not None:
            for idx, road in roads_gdf.iterrows():
                ...
                if geom.geom_type == 'Point':
                    ...
                    cluster_road_points.extend(coords)
                # Skip LineString processing
        ...
        # Get all road points for routing (points only)
        cluster_road_points = []
        if roads_gdf is not None:
            for idx, road in roads_gdf.iterrows():
                geom = road.geometry
                if geom.geom_type == 'Point':
                    cluster_road_points.append((geom.x, geom.y))
        ...
        for cluster_id in range(n_clusters):
            ...
            if cluster_road_points and house_locations_wgs84 and len(house_locations_wgs84) > 0:
                ...
                if len(unique_house_points) >= 1:
                    ...
                    route_coordinates = [[pt[0], pt[1]] for pt in route_points]
            
            if not route_coordinates:
                # Fallback: create route around depot or cluster outline
                if house_locations_wgs84:
                    route_coordinates = cluster_outline
                else:
                    # Just depot point if no buildings
                    route_coordinates = [depot_point]
            ...
            # Create cluster outline Polygon feature
            cluster_polygon_feature = {
                "type": "Feature",
                "geometry": {
                    "type": "Polygon",
                    "coordinates": [cluster_outline]
                },
                "properties": {
                    "feature_type": "cluster_outline",
                    "cluster_id": cluster_id + 1,
                    "vehicle_number": vehicle_number,
                    "vehicle_id": vehicle_id,
                    "house_count": len(cluster_buildings)
            ...
            features.append(cluster_polygon_feature)
            
            # Create depot Point feature
            depot_feature = {
                "type": "Feature",
                "geometry": {
                    "type": "Point",
                    "coordinates": [depot_point[0], depot_point[1]]
                },
                "properties": {
                    "feature_type": "depot",
                    "cluster_id": cluster_id + 1,
...
def generate_map_from_files(ward_file, roads_file, buildings_file):
    ...
    if ward_gdf.empty or roads_gdf.empty or buildings_gdf.empty:
    ...
    # Validate geometries are not null
    ward_gdf = ward_gdf[ward_gdf.geometry.notna()]
    roads_gdf = roads_gdf[roads_gdf.geometry.notna()]
    buildings_gdf = buildings_gdf[buildings_gdf.geometry.notna()]
    
    if ward_gdf.empty or roads_gdf.empty or buildings_gdf.empty:
        raise ValueError("Invalid geometries found in input files")
    ...
    # Get center coordinates from ward bounds with validation
    ward_bounds = ward_gdf.total_bounds
    buildings_bounds = buildings_gdf.total_bounds
    
    # Use buildings bounds if ward bounds are invalid
    if (ward_bounds[2] - ward_bounds[0]) < 0.001 or (ward_bounds[3] - ward_bounds[1]) < 0.001:
        bounds = buildings_bounds
    else:
        bounds = ward_bounds
    ...
    if abs(center_lat) > 90 or abs(center_lon) > 180:
        # Fallback to buildings center
        center_lat = (buildings_bounds[1] + buildings_bounds[3]) / 2
        center_lon = (buildings_bounds[0] + buildings_bounds[2]) / 2
    ...
    # Debug: Print map parameters
    print(f"Map Center: [{center_lat:.6f}, {center_lon:.6f}]")
    print(f"Bounds: {bounds}")
    print(f"Zoom Level: {zoom_level}")
    print(f"Buildings count: {len(buildings_gdf)}")
    
    # Create map with calculated center and zoom
    m = folium.Map(location=[center_lat, center_lon], zoom_start=zoom_level)
    
    # Fit bounds to ensure all data is visible
    try:
        m.fit_bounds([[bounds[1], bounds[0]], [bounds[3], bounds[2]]])
        print("Map bounds fitted successfully")
    except Exception as e:
        print(f"Fit bounds failed: {e}")
        pass  # Continue if fit_bounds fails
    ...
    # Convert to projected CRS for accurate centroid calculation
    buildings_projected = buildings_gdf.to_crs('EPSG:3857')
    building_centroids_proj = [(pt.x, pt.y) for pt in buildings_projected.geometry.centroid]
    ...
    n_clusters = min(5, len(building_centroids))  # Can't have more clusters than buildings
    ...
    # Build road network for routing (points only)
    for idx, road in roads_gdf.iterrows():
        geom = road.geometry
        if geom.geom_type == 'Point':
            # Add point to graph as isolated node
    ...
    for cluster_id in range(5):
        ...
        # Find all road points for routing (points only)
        cluster_road_points = []
        for idx, road in roads_gdf.iterrows():
            geom = road.geometry
            if geom.geom_type == 'Point':
            ...
            for house_pt in house_locations_wgs84:
                if cluster_road_points:
                    ...
                    if distances:  # Check if distances list is not empty
                        ...
                        house_road_points.append(house_pt)  # Use house location if no roads
                else:
                    house_road_points.append(house_pt)  # Use house location if no roads
